	...
	for (u32 j = 0; j < mc; j++) {
		...
		switch(buf->getVertexType()) {
			case video::EVT_STANDARD: {
				video::S3DVertex *v = (video::S3DVertex *) buf->getVertices();
			...
			case video::EVT_2TCOORDS: {
				video::S3DVertex2TCoords *v = (video::S3DVertex2TCoords *) buf->getVertices();
				for (u32 i = 0; i < vertex_count; i++) { 
					v[i].Pos *= scale;
				}
				break;
			...
			case video::EVT_TANGENTS: {
				video::S3DVertexTangents *v = (video::S3DVertexTangents *) buf->getVertices();
				for (u32 i = 0; i < vertex_count; i++) { 
					v[i].Pos *= scale;
				}
				break;
...
void shadeMeshFaces(scene::IMesh *mesh)
{
	...
	for (u32 j = 0; j < mc; j++) {
		...
		switch(buf->getVertexType()) {
			case video::EVT_STANDARD: {
				video::S3DVertex *v = (video::S3DVertex *)
					buf->getVertices();
				for (u32 i = 0; i < vertex_count; i++) { 
					video::SColor &vc = v[i].Color;
					if (v[i].Normal.Y < -0.5) {
						applyFacesShading (vc, 0.447213);
					} else if (v[i].Normal.Z > 0.5) {
						applyFacesShading (vc, 0.670820);
					} else if (v[i].Normal.Z < -0.5) {
						applyFacesShading (vc, 0.670820);
					} else if (v[i].Normal.X > 0.5) {
